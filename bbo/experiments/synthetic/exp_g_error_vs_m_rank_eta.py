"""Exp G: Mean error vs m for varying rank r with fixed label noise eta.

Fix eta=0.1, n=100 models, uniform Pi_Q, p=0.3 (rho=0.7).
Sweep r in {3, 5, 10}.
Expected: all curves converge to L* = eta = 0.1, larger r needs more queries.
"""
...
from bbo.experiments.config import ExpGConfig
...
def run_exp_g(config: ExpGConfig = None) -> pd.DataFrame:
    """Run full Exp G sweep with parallel reps."""
    if config is None:
        config = ExpGConfig()
