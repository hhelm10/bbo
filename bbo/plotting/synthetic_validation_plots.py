...
def plot_synthetic_validation_mean_error(
    df_exp1, df_exp2, df_panel_c, df_exp3,
    df_panel_e, df_panel_f, df_panel_g,
    output_path="figures/figure_synthetic_validation_mean_error.pdf",
):
    """Same as plot_synthetic_validation but top row uses mean_error instead of P[err>=0.5]."""
    set_paper_style()
    ...
    # ===== Panel (a): Query distribution — mean error =====
    df_d = df_exp3[df_exp3["rho"] == 0.7] if "rho" in df_exp3.columns else df_exp3
    dist_labels = {"signal": "Signal", "uniform": "Uniform",
                   "orthogonal": "Orthogonal"}
    dist_colors = {"signal": PALETTE[0], "uniform": PALETTE[2],
    ...
    for dist_name, label in dist_labels.items():
        ...
        if sub.empty:
            continue
        ax_a.plot(sub["m"], sub["mean_error"], marker="o",
                  color=dist_colors[dist_name], label=label)
    ...
    # ===== Panel (b): Varying r — mean error =====
    r_values = sorted(df_exp1["r"].unique())
    for i, r in enumerate(r_values):
        color = PALETTE[i % len(PALETTE)]
        sub = df_exp1[df_exp1["r"] == r]
        rho = sub["rho"].iloc[0]
        ax_b.plot(sub["m"], sub["mean_error"], marker="o", color=color,
                  label=f"$r = {r}$")
        bound = _theory_bound(m_dense, r, rho)
        ax_b.plot(m_dense, bound, color=color,
                  linestyle="--", linewidth=1.0, alpha=0.7)
    ...
    # ===== Panel (c): Varying ρ — mean error =====
    rho_values = sorted(df_exp2["rho"].unique())
    for i, rho in enumerate(rho_values):
        color = PALETTE[i % len(PALETTE)]
        sub = df_exp2[df_exp2["rho"] == rho]
        ax_c.plot(sub["m"], sub["mean_error"], marker="o", color=color,
                  label=f"$\\rho = {rho:.1f}$")
        bound = _theory_bound(m_dense, 5, rho)
        ax_c.plot(m_dense, bound, color=color,
                  linestyle="--", linewidth=1.0, alpha=0.7)
    ...
    # ===== Panel (d): Varying n — mean error (or prob_high_error if unavailable) =====
    n_values_c = sorted(df_panel_c["n_models"].unique())
    rho_c = df_panel_c["rho"].iloc[0]
    r_c = int(df_panel_c["r"].iloc[0])
    _y_col_d = "mean_error" if "mean_error" in df_panel_c.columns else "prob_high_error"

    for i, n_val in enumerate(n_values_c):
        color = PALETTE[i % len(PALETTE)]
        sub = df_panel_c[df_panel_c["n_models"] == n_val].sort_values("m")
        ax_d.plot(sub["m"], sub[_y_col_d], marker="o", color=color,
                  label=f"$n = {n_val}$")

    bound_c = _theory_bound(m_dense, r_c, rho_c)
    ax_d.plot(m_dense, bound_c, color="gray",
              linestyle="--", linewidth=1.0, alpha=0.7)

    hc, lc = ax_d.get_legend_handles_labels()
    hc.append(theory_handle); lc.append("$r\\rho^m$")
    ax_d.set_xscale("log")
    ax_d.set_ylim(-0.02, 0.55)
    ax_d.set_yticklabels([])
    ax_d.set_title(f"(d) Varying $n$\n$r\\!=\\!{r_c},\\; \\rho\\!=\\!{rho_c}$",
                   fontsize=7)
    ax_d.legend(hc, lc, loc="lower left", bbox_to_anchor=(0.02, 0))

    # ===== Bottom row: same as original (estimation panels) =====
    # Panel (e): Rank recovery
    r_values_e = sorted(df_panel_e["r"].unique())
    for i, r in enumerate(r_values_e):
        color = PALETTE[i % len(PALETTE)]
    ...
    # Panel (g): Predicted vs empirical — mean error version
    rho_values_g = sorted(df_panel_g["rho_true"].unique())
    n_reps = int(df_panel_g["n_reps"].iloc[0]) if "n_reps" in df_panel_g.columns else 1000
    for i, rho in enumerate(rho_values_g):
        color = PALETTE[i % len(PALETTE)]
        sub = df_panel_g[df_panel_g["rho_true"] == rho].sort_values("m")

        # Empirical mean error (solid)
        if "mean_error" in sub.columns:
            ax_g.plot(sub["m"], sub["mean_error"], marker="o", color=color,
                      linestyle="-", linewidth=0.8)
        else:
            y_emp = sub["prob_high_error"].values
            ax_g.plot(sub["m"], y_emp, marker="o", color=color,
                      linestyle="-", linewidth=0.8)

        # Predicted from estimated params (dotted)
        y_pred = sub["predicted_bound_mean"].values
        ax_g.plot(sub["m"], y_pred, color=color,
                  linestyle=":", linewidth=1.0, alpha=0.8)

        # True bound (dashed)
        y_true = sub["true_bound"].values
        ax_g.plot(sub["m"], y_true, color=color,
                  linestyle="--", linewidth=1.0, alpha=0.7)

    r_g = int(df_panel_g["r"].iloc[0])
    ax_g.set_xscale("log")
    ax_g.set_ylim(-0.02, 0.55)
    ax_g.set_ylabel("Mean error")
    ax_g.set_xlabel("Queries $m$")
    ax_g.set_title(f"(g) Predicted vs empirical\n$r\\!=\\!{r_g},\\; n\\!=\\!100$",
                   fontsize=7)
